...
def parse_ani_ccx_data(settings, data_keys, device):
    """
    ...
    for molecule in ani_data:
        ...
        # print("conf_indices: ", conf_indices)
        # print("##################check data read from CXX##################")
        # print("molecule['atomic_numbers']: ", molecule['atomic_numbers'].shape)
        # print("molecule['ccsd(t)_cbs.energy']: ", molecule['ccsd(t)_cbs.energy'].shape)
        # print("molecule['coordinates']: ", molecule['coordinates'].shape)

        # If no test data specified, for each molecule split conformations into train and test
        # Modification
        # # incase where the n_conf is 1, we only have train set
        test_size = math.ceil(test_size_per_molecule * n_conf)
        if test_data is False and test_size > 1: 
            # print("test size: ", math.ceil(test_size_per_molecule * n_conf))
            # print("n_conf: ", n_conf)
            train_idx, test_idx = train_test_split(conf_indices, 
                test_size=test_size, 
                random_state=settings['data']['train_test_split_random_state'])
